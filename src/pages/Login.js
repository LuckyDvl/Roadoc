      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
        ...
        {(!process.env.REACT_APP_SUPABASE_URL) && (
          <p className="text-yellow-600 bg-yellow-50 p-2 rounded text-xs mb-4 text-center border border-yellow-200">
            ⚠️ Supabase keys missing in .env. Login will fail.
          </p>
        )}
        ...
        <p className="mt-6 text-center text-sm text-gray-600">
          Don't have an account? <Link to="/register" className="text-yellow-600 font-semibold hover:underline">Register</Link>
        </p>
      </div>
    </div>
  );
